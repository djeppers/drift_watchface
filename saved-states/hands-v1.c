...
#define HAND_SOLID_HW     9   // perpendicular px that stay fully solid
...
static void canvas_update_proc(Layer *layer, GContext *ctx) {
  ...
  draw_hand(ctx, min_angle, -MIN_TAIL_LEN, MIN_TIP_LEN, 12);

  // Hour hand
  int32_t hour_angle =
    (TRIG_MAX_ANGLE * ((now->tm_hour % 12) * 3600 +
                        now->tm_min * 60 + now->tm_sec)) / (12 * 3600);
  draw_hand(ctx, hour_angle, HOUR_INNER, HOUR_OUTER, 10);
